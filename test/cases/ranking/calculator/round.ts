...
import { jsonBaseCompetition, jsonCompetitionScoresOnly, jsonMultiSportsCompetition } from '../../../data/competition';
...
describe('RoundRankingCalculator', () => {
    ...
    it('pointsCalculation points', () => {
        const competition = getCompetitionMapper().toObject(jsonCompetitionScoresOnly);
        const structureEditor = getStructureEditor();
        const structure = structureEditor.create(competition, [4], createPlanningConfigNoTime());
        const rootRound: Round = structure.getRootRound();
